...
void SimpleRenderer::Draw()
{
    ...
    mat4 modelMatrix = translate(mat4(), camera.playerPosition)*glm::rotate(mat4(), camera.anglegun, vec3(0.0f, 1.0f, 0.0f));//translate(mat4(), camera.position - vec3(0.0, 5.0, -10.0))*
	...
	glBindVertexArray(VAOIdVec[step % 8]);
	glDrawElements(GL_TRIANGLES, numIndicesVec[step % 8], GL_UNSIGNED_SHORT, (void*)(IndexDataByteOffsetVec[step % 8]));
	...
	mat4 modelToWorldMatrix = translate(mat4(), vec3(0.0,15.0,0.0)) * scale(mat4(), vec3(30.0f, 30.0f, 30.0f));
	glUniformMatrix4fv(mModelUniformLocation, 1, GL_FALSE, &(modelToWorldMatrix[0][0]));

	glBindVertexArray(VAOIdVec[8]);
	glDrawElements(GL_TRIANGLES, numIndicesVec[8], GL_UNSIGNED_SHORT, (void*)(IndexDataByteOffsetVec[8]));
	...
	glBindVertexArray(VAOIdVec[9]);
	glDrawElements(GL_TRIANGLES, numIndicesVec[9], GL_UNSIGNED_SHORT, (void*)(IndexDataByteOffsetVec[9]));

    mDrawCount += 1;
}

void SimpleRenderer::mouseMoveEvent(int& x, int& y) {
	camera.mouseUpdate((float)x);
	//camera.keyUpdate((float)y);
}
