...
static int engine_init_display(struct engine* engine) {
	...
	* gefahrlos mithilfe von  "EGL_NATIVE_VISUAL_ID" entsprechend neu konfigurieren. */
	...
	shapeExternVec.resize(numberOfObj);
	ShapeData shape = load_geometry(engine, "step1.txt");
	shapeExternVec[0] = shape;
    shape = load_geometry(engine, "step2.txt");
	shapeExternVec[1] = shape;
	shape = load_geometry(engine, "step3.txt");
	shapeExternVec[2] = shape;
	shape = load_geometry(engine, "step4.txt");
	shapeExternVec[3] = shape;
	shape = load_geometry(engine, "step5.txt");
	shapeExternVec[4] = shape;
	shape = load_geometry(engine, "step6.txt");
	shapeExternVec[5] = shape;
	shape = load_geometry(engine, "step7.txt");
	shapeExternVec[6] = shape;
	shape = load_geometry(engine, "step8.txt");
	shapeExternVec[7] = shape;
	shape = load_geometry(engine, "sky2.txt");
	shapeExternVec[8] = shape;
...
static void engine_draw_frame(struct engine* engine) {
	...
	if (run) {
		engine->mCubeRenderer->runEvent(engine->state.y, engine->height);
		if (engine->mCubeRenderer->mDrawCount % 10 == 0) {
			if (engine->state.y > engine->height / 2) {
				engine->mCubeRenderer->step += 1;
			}
			if (engine->state.y < engine->height / 2) {
				if (engine->mCubeRenderer->step > 0) {
					engine->mCubeRenderer->step -= 1;
				}
				else {
					engine->mCubeRenderer->step += 7;
				}
			}
		}
	}
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
	...
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
		...
		if (action == AMOTION_EVENT_ACTION_MOVE) {
			...
			engine->mCubeRenderer->mouseMoveEvent(engine->state.x, engine->state.y);
			//engine->mCubeRenderer->Draw();
		}
